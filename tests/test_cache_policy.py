"""Smoke tests for cache policy and cache store."""
...
class TestHeuristicCachePolicy(unittest.TestCase):
    ...
    def test_decide_reuse_when_within_limits(self):
        ...
        self.assertTrue(
            policy.decide({"num_paths": 10000, "volatility": 0.2})
        )
        self.assertFalse(
            policy.decide({"num_paths": 200000, "volatility": 0.2})
        )
        self.assertFalse(
            policy.decide({"num_paths": 10000, "volatility": 0.5})
        )
