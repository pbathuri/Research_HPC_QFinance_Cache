"""Build simple feature dicts consumed by ``cache_policy`` and ``SimpleCacheStore``.

Keys align with what ``HeuristicCachePolicy`` and ``LogisticCachePolicy`` expect
(``instrument_type``, ``num_paths``, ``volatility``, ``maturity``).
"""
