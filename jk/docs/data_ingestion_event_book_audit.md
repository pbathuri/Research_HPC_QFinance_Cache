...
- **Workflow visualization (simulated)**: `research_agents.py` and `tools/pixel_agents_bridge/` export JSON/JSONL traces without coupling to VS Code or Pixel Agents at import time.
...
- Large multi-agent LLM frameworks; live Pixel Agents ingestion (bridge remains **export-only**).
...
- **Visualization**: `workflow_events.py` emits structured steps; `tools/pixel_agents_bridge/pixel_trace_exporter.py` writes Pixel-friendly JSONL under `outputs/data_ingestion_event_book/`.
